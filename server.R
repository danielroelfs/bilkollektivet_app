...
shinyServer(function(input, output) {
  ...
  pricelist <- "https://bilkollektivet.no/priser/" |> 
    ...
    as_tibble() |> 
    separate(col = value, 
             into = c("cartype", "startgebyr", "price_hour", "price_day", "price_week", "price_km"),
             sep = "[[:space:]]{12}") |> 
    mutate(
      across(everything(), str_trim),
      price_week = str_remove_all(price_week, "1 uke:"),
      across(startgebyr:last_col(), 
             ~ parse_number(.x, locale = locale(decimal_mark = ",", 
                                                grouping_mark = " ")))
    ) |> 
    separate(col = cartype, into = c("carname", "category"), 
             sep ="(?<=[a-z|0-9|L|Y])(?=[A-Z|9])") |> 
    mutate(car = ifelse(str_detect(category, "Budsjett"), 
